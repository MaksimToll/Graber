 ...
 */
public class BeachnceGraberWrapper extends Thread {
    final static Logger logger = Logger.getLogger(BeachnceGraberWrapper.class);
    private static Main main;
    BehanceGrabber thread;
    public BeachnceGraberWrapper(Main main){
        this.main = main;
    }
    @Override
    public void run() {
    ...
    public synchronized void addImage(BufferedImage image, Designer designer, boolean forseWrite) {
        ...
        if ((this.images.size() == BehanceGrabber.expectedImages && BehanceGrabber.expectedImages > 0) || forseWrite) {
            ...
            int imagesInRow = Integer.parseInt( BehanceGrabber.properties.getProperty( "imagesInRow", "2" ) );
